...
function GuruAccountsPage() {
  ...
  // Guru hanya bisa melihat akun mereka sendiri atau di sekolah mereka
  const effectiveSchoolId = selectedSchoolId || user.schoolId;

  // Filter users untuk guru - hanya themselves dan siswa/orang tua di sekolah mereka
  let filteredUsers = users.filter((u: any) => {
    // Guru bisa melihat diri sendiri
    if (u.id === user.id) return true;
    // Atau siswa/orang tua di sekolah sama
    return u.schoolId === effectiveSchoolId && (u.role === 'SISWA' || u.role === 'ORANG_TUA');
  });
...
export default GuruAccountsPage;