...
function SiswaAccountsPage() {
  ...
  // Siswa hanya bisa melihat akun mereka sendiri
  // Atau orang tua/kepala sekolah di sekolah sama
  const effectiveSchoolId = selectedSchoolId || user.schoolId;

  // Filter users untuk siswa - hanya themselves
  // atau orang tua yang memiliki parent relationship
  let filteredUsers = users.filter((u: any) => {
    // Siswa bisa melihat diri sendiri
    if (u.id === user.id) return true;
    // Atau orang tua di sekolah sama
    if (u.role === 'ORANG_TUA' && u.schoolId === effectiveSchoolId) return true;
    return false;
  });
...
export default SiswaAccountsPage;