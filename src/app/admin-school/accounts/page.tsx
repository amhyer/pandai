...
function AdminSchoolAccountsPage() {
  const { users, isLoading, error, selectedSchoolId, setSelectedSchoolId, user } = useAppStore();
  ...
  // Admin School hanya melihat akun di sekolah mereka
  const effectiveSchoolId = selectedSchoolId || user.schoolId;

  // Filter users untuk admin school - hanya guru, siswa, orang tua di sekolah ini
  let filteredUsers = users.filter((u: any) => u.schoolId === effectiveSchoolId);
              ...
              {filteredUsers.map((u: any) => (
                <TableRow key={u.id} className="hover:bg-muted/50">
                  ...
                  <TableCell>
                    {u.role === 'GURU' ? 'Guru' :
                      u.role === 'SISWA' ? 'Siswa' :
                      u.role === 'ORANG_TUA' ? 'Orang Tua' :
                      u.role === 'KEPALA_SEKOLAH' ? 'Kepala Sekolah' : 'Lainnya'}
                  </TableCell>
                  <TableCell>
                    <span
...
export default AdminSchoolAccountsPage;