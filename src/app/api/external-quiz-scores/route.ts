...
export async function GET(req: NextRequest) {
  try {
    ...
    try {
      await logAccess(auth, { action: 'READ', resourceType: 'external-quiz-scores' });
    } catch {}

    if (auth.role === 'KEPALA_SEKOLAH') {
      return NextResponse.json(
        { error: 'Kepala Sekolah hanya dapat mengakses data agregat.' },
        { status: 403 },
      );
    }
      ...
      scores.map(async (s) => {
        const [student, material] = await Promise.all([
          ...
          db.material.findUnique({
            ...
            select: { id: true, title: true, externalProvider: true },
          }),
        ]);
  ...
  try {
    ...
    try {
      await logAccess(auth, {
        action: 'CREATE',
        resourceType: 'external-quiz-scores',
    ...
    if (!materialId || !studentId || score === undefined) {
      return NextResponse.json(
        { error: 'Data wajib belum lengkap (materialId, studentId, score)' },
        { status: 400 },
      );
    }
    ...
    const mode = entryMode || 'SELF_REPORTED';

    if (mode === 'SELF_REPORTED') {
      if (auth.role !== 'SISWA') {
        return NextResponse.json(
          { error: 'Hanya siswa yang dapat melaporkan nilai sendiri' },
          { status: 403 },
        );
      }
      if (studentId !== auth.userId) {
        return NextResponse.json(
          { error: 'Tidak dapat melaporkan nilai untuk siswa lain' },
          { status: 403 },
        );
      }
    } else if (mode === 'TEACHER_ENTERED') {
      if (auth.role !== 'GURU' && auth.role !== 'ADMIN_SCHOOL' && auth.role !== 'SUPER_ADMIN') {
        return NextResponse.json(
          { error: 'Hanya guru yang dapat menginput nilai siswa' },
          { status: 403 },
        );
      }
      await requireStudentScope(auth, studentId);
    } else {
      // Unknown mode — require scope as defense in depth
      await requireStudentScope(auth, studentId);
    }
    ...
    if (existing) {
      result = await db.externalQuizScore.update({
        ...
        data: { score, note: note || null, enteredBy: auth.userId },
      });
    } else {
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    try {
      await logAccess(auth, { action: 'UPDATE', resourceType: 'external-quiz-scores' });
    } catch {}
    ...
    const existing = await db.externalQuizScore.findUnique({
      where: { id },
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    try {
      await logAccess(auth, { action: 'DELETE', resourceType: 'external-quiz-scores' });
    } catch {}
    if (!id) return NextResponse.json({ error: 'ID wajib' }, { status: 400 });
