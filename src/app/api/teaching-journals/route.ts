...
// POST /api/teaching-journals
export async function POST(req: NextRequest) {
  try {
    const body = await req.json();
    const { teacherId, classId, subjectId, schoolId, date, topic, activities, notes } = body;

    if (!teacherId || !date || !topic) {
      return NextResponse.json({ error: 'Guru, tanggal, dan topik wajib diisi' }, { status: 400 });
    }
