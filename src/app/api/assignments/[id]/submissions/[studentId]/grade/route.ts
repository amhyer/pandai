...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string; studentId: string }> }) {
  try {
    ...
    await db.assignmentSubmission.update({
      ...
      data: { score: score !== undefined ? score : totalScore, feedback: feedback || null, status: 'dinilai', gradedAt: new Date() },
    });

    const updated = await db.assignmentSubmission.findUnique({ where: { id: submission.id }, include: { answers: true } });
