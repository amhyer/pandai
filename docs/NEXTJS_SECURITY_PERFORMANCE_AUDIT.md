...
- ✅ Seluruh root dashboard & dynamic `[feature]` route untuk semua role kini Server Component (`force-dynamic`): `/accounts`, `/admin-school`, `/guru`, `/siswa`, `/ortu`, `/kepala-sekolah` serta sub-fitur dinamis memakai `PrefetchedRouteShell` + `getXView` dari URL path, tanpa `useParams` client.
- ✅ Stub `*/accounts` diubah menjadi server pages dengan session pre-load DB.
- ✅ Server-side data fetching dimulai pada Super Admin dashboard: `src/lib/server-dashboard.ts` (`getSuperAdminDashboardData`) melakukan query DB langsung (global analytics + activity logs), dan `/accounts/dashboard` adalah Server Component yang meneruskannya ke `ServerSuperAdminDashboard` + `SuperAdminDashboard(serverData)` sehingga client tidak melakukan analytics fetch kedua.
- ✅ Server-side data fetching pada Admin Sekolah dashboard: `getAdminSchoolDashboardData(schoolId)` langsung membaca analytics sekolah + tryout mendatang, `/admin-school/dashboard` adalah Server Component yang menyediakan data melalui `ServerAdminSchoolDashboard` + `AdminSekolahDashboard(serverData)`, sehingga client tidak fetch `/api/analytics` / `/api/exams` saat pertama kali.
- ✅ Server-side data fetching pada Guru dashboard: `getGuruDashboardData(schoolId)` membaca jumlah soal (global + sekolah), tryout, rata-rata nilai, aktivitas terbaru, serta top students; `/guru/dashboard` adalah Server Component yang meneruskannya via `ServerGuruDashboard` + `GuruDashboard(serverData)`.
- ✅ Server-side data fetching pada Siswa dashboard: `getSiswaDashboardData(userId)` membaca analitik siswa (last score, total tryout, avg correct, rank, weak topics, score trend, subject breakdown); `/siswa/dashboard` adalah Server Component via `ServerSiswaDashboard` + `SiswaDashboard(serverData)`.
- ✅ Server-side data fetching pada Orang Tua dashboard: `getOrtuDashboardData(userId)` membaca daftar anak + analitik + kehadiran (hanya status hari sekolah) di server; `/ortu/dashboard` via `ServerOrtuDashboard` + `OrangTuaDashboard(serverData)`.
- ✅ Server-side data fetching pada Kepala Sekolah dashboard: logika kueri `/api/kepsek/dashboard` diangkat ke `getKepalaSekolahDashboardData(schoolId)` (rekap kelas, guru, kebiasaan, kebiasaan per kelas) dan endpoint legacy kini memakai loader yang sama; `/kepala-sekolah/dashboard` via `ServerKepalaSekolahDashboard` + `KepalaSekolahDashboard(serverData)`.
- ✅ Per-feature App Router boundaries (`loading.tsx`, `error.tsx`, `not-found.tsx`) ditambahkan di `[feature]` untuk seluruh role (`accounts`, `admin-school`, `guru`, `siswa`, `ortu`, `kepala-sekolah`).
- 🔲 Refactor penuh: memindahkan data fetching per-feature ke Server Components untuk fitur lain (nilai, bank soal, materi, kehadiran, laporan) masih perlu sprint terpisah, karena membutuhkan build/test yang bisa dijalankan untuk memverifikasi perilaku session & layout.
