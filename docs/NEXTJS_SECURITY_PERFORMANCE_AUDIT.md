...
- 🔲 Refactor penuh: memindahkan data fetching per-feature ke Server Components untuk dashboard role lain/lebih banyak fitur (nilai, bank soal, materi, kehadiran, laporan) serta membuat `loading.tsx`/`error.tsx` per fitur masih perlu sprint terpisah, karena membutuhkan build/test yang bisa dijalankan untuk memverifikasi perilaku session & layout.
